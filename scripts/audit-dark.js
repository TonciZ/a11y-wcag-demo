...
const AUDITS_DIR = path.join(process.cwd(), '.planning', 'audits', 'dark');
const PORT = 8765;
...
// Run pa11y with axe runner on a single URL (for axe-style results) with dark mode injection
async function runPa11yAxe(url) {
  try {
    const { stdout, stderr } = await new Promise((resolve, reject) => {
      exec(
        `pa11y "${url}" --runner axe --reporter json --include-notices --wait 500`,
...
// Run pa11y with htmlcs runner on a single URL with dark mode injection
async function runPa11yHtmlcs(url) {
  try {
    const { stdout, stderr } = await new Promise((resolve, reject) => {
      exec(
        `pa11y "${url}" --runner htmlcs --reporter json --include-notices --wait 500`,
        { timeout: 30000, maxBuffer: 10 * 1024 * 1024 },
        (error, stdout, stderr) => {
          if (error && error.code !== 2) {
            reject(error);
...
async function auditCheckpoints() {
  ...
  try {
    ...
    console.log(`Scanning ${files.length} checkpoint files in DARK mode...`);
    console.log(`Note: Dark mode theme injection enabled via localStorage and data-theme attribute`);
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      console.log(`[${i + 1}/${files.length}] Scanning ${name} (DARK MODE)...`);
